  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-hidden">
      ...
      <section className="h-screen flex flex-col justify-center px-6 md:px-12 relative z-10">
        ...
        >
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tighter leading-tight text-white drop-shadow-2xl">
            <InteractiveHello /> <span className="">Hope your </span>
            <motion.span
              className="relative inline-block cursor-pointer group"
              onMouseEnter={() => setIsTimeHovering(true)}
              onMouseLeave={() => setIsTimeHovering(false)}
            >
              <span className="text-primary underline decoration-wavy underline-offset-8 decoration-2">{dayPart}</span>
              <AnimatePresence>
            ...
            >
              <span className="relative z-10">Glib</span>
              <motion.span 
                className="absolute inset-x-0 bottom-2 h-4 bg-primary/30 -z-10 rounded-full blur-sm"
                initial={{ scaleX: 0 }}
                animate={{ scaleX: 1 }}
              ...
              <AnimatePresence>
                {isNameHovering && (
                  <motion.div
                    ...
                    className="absolute left-1/2 -translate-x-1/2 bottom-full mb-4 z-50 bg-black/60 backdrop-blur-xl border border-white/10 px-6 py-3 rounded-2xl shadow-2xl whitespace-nowrap text-left font-normal tracking-normal leading-normal text-lg"
                  >
                    <span className="text-white/90 font-mono">{displayedName}<span className="animate-pulse text-primary">_</span></span>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.span>
            ...
            <motion.span 
              ...
              className="inline-flex items-center text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-400 relative cursor-pointer group h-[1.2em] align-middle"
              ...
              <AnimatePresence>
                {isHovering && (
                  <motion.div
                    ...
                    className="absolute left-full top-1/2 -translate-y-1/2 ml-8 z-50 bg-black/60 backdrop-blur-xl border border-white/10 p-6 rounded-3xl shadow-2xl min-w-[340px] hidden lg:block text-left font-normal tracking-normal leading-normal"
                  >
          ...
          <motion.div
            ...
            className="mt-8 flex items-center gap-3 text-lg md:text-xl text-white/60 font-light"
          >
        ...
        >
          <Link href="/photo" className="group flex items-center gap-3 text-white/80 hover:text-primary transition-colors font-mono uppercase text-sm tracking-widest bg-black/20 backdrop-blur-md px-6 py-3 rounded-full border border-white/10 hover:border-primary/50">
      ...
      >
        {[
          { title: "Photography", href: "/photo", color: "from-pink-500/20 to-purple-500/20", hoverColor: "group-hover:from-pink-500/40 group-hover:to-purple-500/40" },
          { title: "Videography", href: "/video", color: "from-cyan-500/20 to-blue-500/20", hoverColor: "group-hover:from-cyan-500/40 group-hover:to-blue-500/40" },
          { title: "Motion", href: "/motion", color: "from-yellow-500/20 to-orange-500/20", hoverColor: "group-hover:from-yellow-500/40 group-hover:to-orange-500/40" },
        ].map((item, index) => (
...
function TiltCard({ item, index }: { item: any; index: number }) {
  ...
  return (
    ...
    >
      <Link
        href={item.href}
        className="group relative flex h-full w-full items-center justify-center rounded-[2rem] border border-white/5 hover:border-white/20 transition-all duration-500 bg-black/40 backdrop-blur-sm"
      ...
      >
        ...
        <span className="absolute bottom-8 left-8 font-mono text-xs text-white/40 group-hover:text-white transition-colors border border-white/10 px-3 py-1 rounded-full" style={{ transform: "translateZ(40px)" }}>
          0{index + 1}
        </span>
...
function InteractiveHello() {
  ...
  return (
    <motion.span
      ...
      whileTap={{ scale: 0.95 }}
      onClick={handleClick}
      className="inline-block hover:text-primary transition-colors select-none"
      style={{ cursor: `url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='64' height='64' viewport='0 0 100 100' style='fill:black;font-size:48px;'><text y='50%' font-size='48'>👋</text></svg>") 16 0, auto` }}
    >
      Hello!
    </motion.span>
  )
